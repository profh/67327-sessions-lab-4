...
class DB
  class << self
    ...
    def create_users(session_id)
      ...
      user = User.new(username:'admin', password:'admin', first_name:'Store', last_name:'Owner', registration_key:SecureRandom.hex(25), is_admin:true, is_active:true)
      user.session_id = session_id
      user.save!
      
      # user to find with brute force
      user = User.new(username:'mheimann', password:'qwerty', first_name:'Mark', last_name:'Heimann', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      
      # another user for for potential brute force
      user = User.new(username:'lheimann', password:'john316', first_name:'Trying to hack me?', last_name:'You are in trouble!', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      
      # another user for for potential brute force
      # user = User.new(username:'aheimann', password:'qwerty', first_name:'Alex', last_name:'Heimann', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      # user.session_id = session_id
      # user.save!
      
      # another user for for potential brute force
      user = User.new(username:'raja', password:'letmein', first_name:'Raja', last_name:'Sooriamurthi', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      
      # another user for for potential brute force
      user = User.new(username:'jquesenberry', password:'abc123', first_name:'Jeria', last_name:'Quesenberry', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      
      # another user for for potential brute force
      user = User.new(username:'egruberman', password:'password', first_name:'Ed', last_name:'Gruberman', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
      
      # another user for for potential brute force
      user = User.new(username:'tgruberman', password:'123456', first_name:'Ted', last_name:'Gruberman', registration_key:SecureRandom.hex(25), is_admin:false, is_active:true)
      user.session_id = session_id
      user.save!
